    ...
    FilterChip(
        ...
        border = FilterChipDefaults.filterChipBorder(
            ...
            disabledBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = HRKChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            disabledSelectedBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = HRKChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            selectedBorderWidth = HRKChipDefaults.ChipBorderWidth,
        ),
        colors = FilterChipDefaults.filterChipColors(
            ...
            disabledContainerColor = if (selected) {
                MaterialTheme.colorScheme.onBackground.copy(
                    alpha = HRKChipDefaults.DISABLED_CHIP_CONTAINER_ALPHA,
                )
            } else {
                Color.Transparent
            },
...
fun ChipPreview() {
    HRKTheme {
        HRKBackground(modifier = Modifier.size(80.dp, 20.dp)) {
            HRKFilterChip(selected = true, onSelectedChange = {}) {
                Text("Chip")
            }
        }
    }
